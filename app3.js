...
barba.hooks.afterEnter(data => {
  // Fermer le menu (masqué visuellement par les pixels)
...
function initFooterParallax(container = document){
  container.querySelectorAll('[data-footer-parallax]').forEach(el => {
    if (el.__parallaxInit) return;
    el.__parallaxInit = true;
...
function initRectangleReveal(container = document) {
  ...
  container.querySelectorAll('[data-effect="swipe-reveal"]').forEach((text) => {
    if (text.__swipeRevealInit) return;
    text.__swipeRevealInit = true;
    const wrapper = document.createElement("div");
    wrapper.style.position = "relative"; wrapper.style.display = "inline-block"; wrapper.style.overflow = "hidden";
    text.parentNode.insertBefore(wrapper, text); wrapper.appendChild(text);
    ...
    overlay.style.backgroundColor = CONFIG.color; overlay.style.transform = "scaleX(0)"; overlay.style.transformOrigin = "left"; overlay.style.zIndex = "2";
...
function addMagneticEffect(container = document) {
  ...
  stickers.forEach((sticker) => {
    if (sticker.__magneticInit) return; 
    sticker.__magneticInit = true;
...
function initDraggableStickers(container = document) {
  ...
  stickers.forEach(sticker => {
    if (sticker.__dragInit) return; 
    sticker.__dragInit = true;
...
function initScrambleOnLoad(container = document){
  container.querySelectorAll('[data-scramble="load"]').forEach((target) => {
    if (target.__scrambleLoadInit) return;
    target.__scrambleLoadInit = true;
...
function initScrambleOnScroll(container = document){
  container.querySelectorAll('[data-scramble="scroll"]').forEach((target) => {
    if (target.__scrambleScrollInit) return;
    target.__scrambleScrollInit = true;
...
function initScrambleOnHover(container = document){
  container.querySelectorAll('[data-scramble-hover="link"]').forEach((target) => {
    if(target.__scrambleHoverInit) return; 
    target.__scrambleHoverInit = true;
    ...
    target.addEventListener("mouseenter", () => { gsap.to(textEl, { duration: 1.2, speed: 0.5, scrambleText: { text: customHoverText ? customHoverText : originalText, chars: "ABCDEFGHIJKLMNOPQRSTUVWXYZ&@#" } }); });
    target.addEventListener("mouseleave", () => { gsap.to(textEl, { duration: 1, scrambleText: { text: originalText, speed: 1, chars: "ABCDEFGHIJKLMNOPQRSTUVWXYZ&@#" } }); });
  });
}
...
function initCSSMarquee(container = document) {
  ...
  marquees.forEach(marquee => {
    if(marquee.__marqueeInit) return;
  ...
  marquees.forEach(marquee => {
    marquee.querySelectorAll('[data-css-marquee-list]').forEach(list => { list.style.animationDuration = (list.offsetWidth / pixelsPerSecond) + 's'; list.style.animationPlayState = 'paused'; });
...
function initTwostepScalingNavigation() {
  ...
  if (navElement.__navInit) return;
  navElement.__navInit = true;
...
function initMwg026Effect(pageContainer = document) {
  pageContainer.querySelectorAll('.mwg026').forEach(root => {
    ...
    if (!container) return;
    const halfX = container.clientWidth / 2; const wrapX = gsap.utils.wrap(-halfX, 0); const xTo = gsap.quickTo(container, 'x', { duration: 1.5, ease: "power4", modifiers: { x: gsap.utils.unitize(wrapX) } });
    const halfY = container.clientHeight / 2; const wrapY = gsap.utils.wrap(-halfY, 0); const yTo = gsap.quickTo(container, 'y', { duration: 1.5, ease: "power4", modifiers: { y: gsap.utils.unitize(wrapY) } });
    let incrX = 0, incrY = 0;
    Observer.create({
...
function initMouseTrailEffect(container = document) {
  container.querySelectorAll('.mwg020').forEach(root => {
    ...
    root.addEventListener("mousemove", e => {
      ...
      if(incr > resetDist) { incr = 0; createMedia(valX, valY - root.getBoundingClientRect().top, valX - oldIncrX, valY - oldIncrY); }
...
function initFooterDeformation(container = document) {
  container.querySelectorAll('.effet-deformation').forEach(el => {
    ...
    el.addEventListener('mousemove', (e) => {
      ...
      const skewX = distX * 5; const skewY = distY * 5;
...
/* TITLE REVEAL (data-animate="title-reveal") */
